...
test_that('Row Hamming sparse similarity calculation', {
  ...
  sim_expected <- matrix(c(1.0, 0.5, 0.25, 0.5, 1.0, 0.75, 0.25, 0.75, 1.0), ncol = 3)
  sim_expected <- methods::as(Matrix::Matrix(sim_expected, sparse = TRUE), "symmetricMatrix")
  expect_equal(sim_blocksR(X, metric = 'hamming'), sim_expected)
  expect_equal(sim_blocksR(X, metric = 'hamming', row_blocks = 3), sim_expected)
  
  sim_expected[sim_expected < 0.75] <- 0
  sim_expected <- methods::as(sim_expected, "symmetricMatrix")
  expect_equal(sim_blocksR(X, metric = 'hamming', thresh = 0.75), sim_expected)
  expect_equal(sim_blocksR(X, metric = 'hamming', thresh = 0.75, row_blocks = 3), sim_expected)
})
