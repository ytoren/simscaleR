...
We usually have some idea of the maximal size of items we want to calculate. For this example (running on my laptop) we will stop at ~25k rows (and try in steps of 1000). If you're not sure about the machine you're going to use you might want to set a higher maximum with less steps just to find the limit. 
...
```{r plot_prop, fig.width=75, fig.height=5}
...
The memory cap we calculated was for a single core using all available memory. In a multi-core scenario we have to consider the fact that memory is shared by all cores on the same machine. We should also consider the fact that most parallelisation mechanisms have some memory overhead per-core (for example a FORK cluster replicated the entire environment per core).
